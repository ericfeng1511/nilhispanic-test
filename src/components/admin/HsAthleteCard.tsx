...
interface HsAthleteCardProps {
  user: HighSchoolAthleteUser;
  onClick?: () => void;
}

export const HsAthleteCard: React.FC<HsAthleteCardProps> = ({ user, onClick }) => {
  ...
  const gradeLabel =
    user.grade === 9 ? '9th Grade' :
    user.grade === 10 ? '10th Grade' :
    user.grade === 11 ? '11th Grade' :
    user.grade === 12 ? '12th Grade' : null;
        ...
        <div className="flex sm:hidden items-center space-x-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between mt-2">
              <span className="text-xs text-gray-500">{user.sport || '—'}</span>
              {gradeLabel && <span className="text-xs text-gray-500">{gradeLabel}</span>}
            </div>
          </div>
        </div>
      </CardContent>
