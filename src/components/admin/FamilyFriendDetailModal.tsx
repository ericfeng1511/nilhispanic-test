...
import { User, MapPin, Instagram, Globe, Mail, Heart } from 'lucide-react';

interface FamilyFriendDetailModalProps {
  user: FamilyFriendUser | null;
  isOpen: boolean;
  onClose: () => void;
}

export const FamilyFriendDetailModal: React.FC<FamilyFriendDetailModalProps> = ({ user, isOpen, onClose }) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="relative aspect-square rounded-lg overflow-hidden bg-gray-100">
            ...
            {!imageError && hasPhoto ? (
              <img
                src={user.photo!}
                alt={user.full_name || 'Family/Friend'}
          ...
          <div className="space-y-4">
            <div className="space-y-3">
              ...
              <div className="flex items-center space-x-3">
                <Heart className="w-5 h-5 text-nil-orange flex-shrink-0" />
                <div>
                  <span className="font-medium text-gray-700">Relationship:</span>
                  <span className="ml-2 text-gray-900">{user.relationship_type || 'N/A'}</span>
                </div>
              </div>
