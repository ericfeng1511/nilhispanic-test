...
const AlumniDashboard: React.FC = () => {
  ...
  const [alumniProfile, setAlumniProfile] = useState<AlumniProfile>({
    age: undefined,
    city_id: undefined,
    hometown: '',
    cultural_roots: [],
  ...
  const { data: currentData, isLoading: dataLoading } = useQuery({
    queryKey: ['alumni-profile', profile?.id],
    queryFn: async () => {
    ...
    enabled: !!profile?.id && profile?.role === 'alumni',
    staleTime: 5 * 60 * 1000,
  });
  ...
  const photoUploadMutation = useMutation({
    mutationFn: async (file: File): Promise<string> => {
      ...
      const filePath = `alumni-photos/${fileName}`;
  ...
  const updateMutation = useMutation({
    mutationFn: async (updates: AlumniProfile) => {
      if (!profile?.id) throw new Error('No profile ID available');
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['alumni-profile', profile?.id] });
  ...
  const handleSave = () => {
    updateMutation.mutate(alumniProfile);
  };
  ...
  const handleCancel = () => {
    if (currentData) {
      ...
      setAlumniProfile({ age: undefined, city_id: undefined, hometown: '', cultural_roots: [], instagram_handle: '' });
    }
    setIsEditing(false);
  ...
  const handleClearAll = () => {
    setAlumniProfile({ age: undefined, city_id: undefined, hometown: '', cultural_roots: [], instagram_handle: alumniProfile.instagram_handle });
    setCityQuery('');
    setSelectedCityLabel('');
  };

  const startSocialEdit = () => {
    setOriginalInstagramHandle(alumniProfile.instagram_handle);
    setIsEditingSocial(true);
  };

  const handleSaveSocial = () => {
    updateSocialMutation.mutate(alumniProfile.instagram_handle);
  };

  const handleCancelSocial = () => {
    setAlumniProfile(prev => ({ ...prev, instagram_handle: originalInstagramHandle }));
    setIsEditingSocial(false);
  };
  ...
  const isProfileComplete = () =>
    typeof alumniProfile.age === 'number' &&
    (selectedCityLabel.trim() !== '' || alumniProfile.hometown.trim() !== '');

  // Open a specific conversation if openChat query param is present
  useEffect(() => {
    const convoId = searchParams.get('openChat');
    if (convoId && profile?.id && profile.role === 'alumni') {
      setIsChatOpen(true);
  ...
  useEffect(() => {
    ...
    if (groupId && profile?.id && profile.role === 'alumni') {
      setIsChatOpen(true);
  ...
  const handleOpenMessages = async () => {
    ...
    try {
      ...
      const res = await ChatService.listConversationsForUser(profile.id, 'alumni', 1, 50);
      setConversations(res.data || []);
      try {
        const previews = await ChatService.getUnreadPreviewsForUser(profile.id, 'alumni');
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center max-w-md mx-auto p-8">
          ...
          <p className="text-gray-600 mb-6">
            You need alumni privileges to access this dashboard.
          </p>
          <button
    ...
    <div className="min-h-screen bg-white">
      <div className="container mx-auto px-4 py-8">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600 mt-1">
                  Manage your ÑIL Hispanic™ Alumni Profile
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
              <div className="relative">
                <div className="w-32 h-32 rounded-full overflow-hidden bg-gray-100 border-4 border-white shadow-lg">
                  {photoPreview || alumniProfile.photo ? (
                    <img
                      src={photoPreview || alumniProfile.photo}
                      alt="Profile"
        ...
        <PhotoCropperModal
          ...
          initialImageUrl={alumniProfile.photo || null}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                {isEditing ? (
                  <Input
                    ...
                    min={1}
                    max={99}
                    value={alumniProfile.age ?? ''}
                    onChange={(e) => {
                      const val = e.target.value;
                      if (val === '') {
                        setAlumniProfile(prev => ({ ...prev, age: undefined }));
                      } else {
                        const n = parseInt(val, 10);
                        if (!isNaN(n) && n > 0) {
                          setAlumniProfile(prev => ({ ...prev, age: n }));
                        }
                      }
                    }}
                  ...
                  <div className="p-3 bg-gray-50 rounded-md">
                    {alumniProfile.age ?? 'Not specified'}
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2 relative">
                ...
                {isEditing ? (
                        ...
                        {citySuggestions.length === 0 ? (
                          ...
                          citySuggestions.map((c) => (
                            <div
                              ...
                              onClick={() => {
                                setAlumniProfile(prev => ({ ...prev, city_id: c.id }));
                  ...
                  <div className="p-3 bg-gray-50 rounded-md">
                    {selectedCityLabel || alumniProfile.hometown || 'Not specified'}
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                {isEditing ? (
                    ...
                    <PopoverTrigger asChild>
                      ...
                      >
                        <span className="truncate text-left">
                          {alumniProfile.cultural_roots && alumniProfile.cultural_roots.length > 0
                            ? `${alumniProfile.cultural_roots.length} selected`
                    ...
                    <PopoverContent className="w-72 p-0">
                      <div className="max-h-60 overflow-y-auto py-1">
                        {CULTURAL_ROOT_OPTIONS.map((opt) => {
                          ...
                          const checked = !!(alumniProfile.cultural_roots || []).includes(opt);
                          return (
                            <button
                              key={opt}
                              type="button"
                  ...
                  <div className="p-3 bg-gray-50 rounded-md">
                    {alumniProfile.cultural_roots && alumniProfile.cultural_roots.length > 0
                      ? alumniProfile.cultural_roots.join(', ')
                      : 'Not specified'}
                  </div>
                )}
              </div>
        ...
        <Card className="bg-white/75 backdrop-blur-sm border-white/20">
          ...
          <CardContent>
            {!alumniProfile.instagram_handle && (
              <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-3">
                <AlertTriangle className="w-5 h-5 text-amber-600 mt-0.5 flex-shrink-0" />
                <div>
              ...
              <div className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50 transition-colors">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {alumniProfile.instagram_handle
                        ? `@${alumniProfile.instagram_handle.replace(/^@+/, '')}`
                        : 'Not connected'}
                    </p>
                  </div>
                </div>
                {isEditingSocial && (
                  <div className="relative">
                    ...
                    <Input
                      value={alumniProfile.instagram_handle || ''}
                      onChange={(e) => {
                        const cleaned = e.target.value.replace(/^@+/, '').replace(/[^a-zA-Z0-9._-]/g, '');
                        setAlumniProfile(prev => ({ ...prev, instagram_handle: cleaned }));
                      }}
