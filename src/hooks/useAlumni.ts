...
import { AlumniService, AlumniUser } from '@/services/alumniService';
...
export const useAlumni = () => {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: AlumniService.fetchAll,
    staleTime: 5 * 60 * 1000,
    gcTime: 10 * 60 * 1000,
  });

  const all: AlumniUser[] = data ?? [];
  ...
  const updateFilters = (newFilters: AlumniFilters) => {
