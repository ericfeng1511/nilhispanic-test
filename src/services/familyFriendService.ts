...
export interface FamilyFriendUser {
  id: string;
...
export class FamilyFriendService {
  static async fetchAll(): Promise<FamilyFriendUser[]> {
    const { data: profiles, error: profilesError } = await supabase
      .from('profiles')
      .select('id, full_name, email, created_at')
      .eq('role', 'family_friend')
    ...
    const { data: details } = await supabase
      .from('family_friends')
      .select('profile_id, relationship_type, age, hometown, instagram_handle, cultural_roots, photo')
    ...
    return profiles.map((p) => ({
      ...
      relationship_type: detailMap[p.id]?.relationship_type ?? null,
